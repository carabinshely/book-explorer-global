...
- `wrangler versions upload` does not apply the Worker's non-versioned subdomain
  setting. Before upload or promotion, the protected mutation job therefore
  reads that setting, reconciles only `enabled=false, previews_enabled=true`
  when needed, and requires an exact readback before continuing. It never adds
  a route, Custom Domain, or DNS record.
