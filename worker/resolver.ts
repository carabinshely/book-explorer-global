...
export type ManifestLink = Readonly<{ id: string; public_path: string | null; revision: number; lifecycle: RuntimeLifecycle; route_eligible: boolean; url?: string; fallback_url?: string }>;
...
export function validateManifest(value: unknown): LinkManifest {
  ...
  const links = value.links.map((candidate, index): ManifestLink => {
    ...
    const allowedKeys = ["id", "public_path", "revision", "lifecycle", "route_eligible", "url", "fallback_url"];
    ...
    const route_eligible = candidate.route_eligible;
    if (!LIFECYCLES.has(lifecycle)) throw new Error(`invalid manifest: links[${index}].lifecycle`);
    if (typeof route_eligible !== "boolean" || (route_eligible && public_path === null)) throw new Error(`invalid manifest: links[${index}].route_eligible`);
    ...
    return { id, public_path, revision: candidate.revision as number, lifecycle, route_eligible, url, ...(fallback_url ? { fallback_url } : {}) };
  });
...
export function resolvePath(manifest: LinkManifest, pathname: string, preview = false): Resolution {
  ...
  // Route approval is intentionally independent from physical-shipment evidence.
  if (!link.route_eligible) return { kind: "gone", recovery: "niran" };
  switch (link.lifecycle) {
    case "shipped": case "active": return redirect();
    case "approved-preview": return preview ? redirect() : { kind: "gone", recovery: "niran" };
    case "approved": return redirect();
