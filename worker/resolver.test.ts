...
describe("link manifest resolver", () => {
  it("accepts the exact compiler fixture in production when its route is eligible and verifies its payload digest", async () => {
    ...
    expect(validateManifest(fixture, true).links[0].route_eligible).toBe(true);
  ...
  it("dispatches every lifecycle and mode", () => {
    ...
    const approved = validateManifest(withLifecycle("approved"), true);
    ...
    const routeIneligible = validateManifest(withLifecycle("approved", { route_eligible: false }), true);
